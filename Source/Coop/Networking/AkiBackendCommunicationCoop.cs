...
{
    public class SptBackendCommunicationCoop : SptBackendCommunication
    {
            ...
            , out Dictionary<string, object> generatedData)
        {
            ...
            //SptBackendCommunication.Instance.SendDataToPool("", data);
            GameClient.SendData(Encoding.UTF8.GetBytes(data.ToJson()));
            //SptBackendCommunication.Instance.PostJson("/coop/server/update", data.ToJson());
            generatedData = data;
        }
    }
}
