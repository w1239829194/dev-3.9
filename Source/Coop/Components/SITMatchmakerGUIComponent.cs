...
namespace StayInTarkov.Coop.Components
{
    ...
    internal class SITMatchmakerGUIComponent : MonoBehaviour
    {
        ...
        private void DeleteExistingMatches()
        {
            ...
            jsonObj.Add("serverId", SptBackendCommunication.Instance.ProfileId);
            SptBackendCommunication.Instance.PostJsonBLOCKING("/coop/server/delete", jsonObj.ToString());
        }
        ...
        void GetMatches()
        {
            ...
            GetMatchesTask = Task.Run(async () =>
            {
                while (!StopAllTasks)
                {
                    var result = await SptBackendCommunication.Instance.PostJsonAsync("/coop/server/getAllForLocation", RaidSettings.ToJson(), timeout: 4000);
                    if (result != null)
                    {
        ...
        private void HostRaidAndJoin()
        {
            ...
            SptBackendCommunication.Instance.PostDownWebSocketImmediately(joinPacket.SITToJson());
