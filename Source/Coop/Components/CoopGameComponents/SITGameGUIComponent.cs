...
namespace StayInTarkov.Coop.Components.CoopGameComponents
{
    public class SITGameGUIComponent : MonoBehaviour
    {
        ...
        private Rect DrawPing(Rect rect)
        {
            ...
            var text = new GUIContent($"{protocol}{(SITMatchmaking.IsClient ? "client" : "host")} ping:{gameclient.Ping} up:{gameclient.UploadSpeedKbps:0.00} down:{gameclient.DownloadSpeedKbps:0.00} loss:{gameclient.PacketLoss:0.00}% {(SptBackendCommunication.Instance.HighPingMode ? "hpm" : "")}");
            GUI.Label(rect, text);
            GUI.contentColor = Color.white;
