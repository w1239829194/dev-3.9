...
using StayInTarkov.SptSupport.Singleplayer.Utils.InRaid;
...
namespace StayInTarkov.Coop.SITGameModes
{
    ...
    public sealed class CoopSITGame : BaseLocalGame<EftGamePlayerOwner>, IBotGame, ISITGame
    {
            ...
            , TimeSpan sessionTime)
        {
            ...
            coopGame.func_1 = (player) => EftGamePlayerOwner.Create<EftGamePlayerOwner>(player, inputTree, insurance, backEndSession, gameUI, coopGame.GameDateTime, location);
        ...
        private IEnumerator ClientLoadingPinger()
        {
            ...
            while (true)
            {
                ...
                SptBackendCommunication.Instance?.PingAsync();
        ...
        public static async Task<ISpawnPoint?> SendOrReceiveSpawnPoint(ISpawnPoint selectedSpawnPoint, SpawnPoints spawnPoints)
        {
            ...
            if (!SITMatchmaking.IsClient)
            {
                ...
                await SptBackendCommunication.Instance.PostJsonAsync("/coop/server/update", packet.ToJson());
            }
            else if (SITMatchmaking.IsClient)
            {
                if (PluginConfigSettings.Instance.CoopSettings.AllPlayersSpawnTogether)
                {
                    var json = await SptBackendCommunication.Instance.GetJsonAsync($"/coop/server/spawnPoint/{SITGameComponent.GetServerId()}");
        ...
        public override void Stop(string profileId, ExitStatus exitStatus, string exitName, float delay = 0f)
        {
            ...
            if (SITMatchmaking.IsServer)
            {
                ...
                foreach (var p in SITGameComponent.GetCoopGameComponent().Players)
                {
                    ...
                    // make sure the host does not "leave game" before other players since Relay has special handling Spt-side
                    if (pid != hostProfileId && !isBot)
                    {
                        SptBackendCommunication.Instance.PostJsonBLOCKING("/coop/server/update", new Dictionary<string, object>() {
                            { "m", "PlayerLeft" },
                            { "profileId", pid },
