...
/***
 * Full Credit for this patch goes to SPT-Spt team
...
namespace StayInTarkov
{
    public static class BundleManager
    {
        ...
        {
            return SptBackendCommunication.IsLocal
        ...
        {
            // get bundles
            var json = await SptBackendCommunication.Instance.GetJsonAsync("/singleplayer/bundles");
            ...
            if (SptBackendCommunication.IsLocal)
            {
            ...
            else
            {
                ...
                foreach (var bundle in toDownload)
                {
                    ...
                    try
                    {
                        ...
                        var data = await SptBackendCommunication.Instance.GetBundleData($"/files/bundle/{bundle.FileName}");
                ...
                foreach (var bundle in failDownload)
                {
                    ...
                    try
                    {
                        ...
                        var data = await SptBackendCommunication.Instance.GetBundleData($"/files/bundle/{bundle.FileName}", 600000);
                        if (data != null && data.Length == 0)
                        {
                            StayInTarkovHelperConstants.Logger.LogError("Bundle received is 0 bytes. WTF!");
                        }
