...
1. Download the server mod’s zip file from [GitHub](https://github.com/stayintarkov/SIT.Spt-Server-Mod) (look for it under the big green button: *Code > Download Zip*).
2. Unzip file and extract contents to `SIT_DIR/server/user/mods`.

    *The `user/mods` directory is automatically created when the server is run the first time. Run `Spt.Server.exe` to create the folder. Stop and close the server once the directory has been created so we can continue the installation process.*
...
### Server
1. Open the coop server configuration file in `SIT_DIR/server/user/mods/SIT.Spt Server-Mod/config/coopConfig.json`.

    *The `coopConfig.json` file is automatically created when the server mod is run the first time. Run `Spt.Server.exe` to create the file. Stop and close the server once the file has been created so we can continue the installation process.*
...
4. Optionally, set `logRequests` to `false` in `SIT_DIR/server/SPT_Data/Server/configs/http.json` to prevent log spam.
...
7. **Update HTTP config**: go to `SIT_DIR\server\SPT_Data\Server\configs` and open `http.json` in a text editor. Change the `ip` value to the static local IP you assigned to your machine. This step will allow you to connect to your server from your own computer
8. **Update co-op config**: go to `SIT_DIR\server\user\mods\SIT.Spt-Server-Mod-master\config` and open `coopConfig.json` in a text editor. Change the `externalIP` value to the IP given by whatismyip. This step will allow your friends to connect to your server.
...
Optionally, set `logRequests` to `false` in `SIT_DIR/server/SPT_Data/Server/configs/http.json` to prevent log spam.
...
3. Open the coop server configuration file in `SIT_DIR/server/user/mods/SIT.Spt Server-Mod/config/coopConfig.json`.
...
6. Open SPT-AKI's server connection configuration file in `SIT_DIR/server/SPT_Data/Server/configs/http.json`.
...
Run `Spt.Server.exe` from `SIT_DIR/server`.
