...
Ein Escape From Tarkov BepInEx Modul für SPT-Spt mit dem Ziel einen "Offline" Coop-Modus zu emulieren.
</div>
...
Als nächstes brauchst du den SIT-Servermod -> [Download](https://github.com/paulov-t/SIT.Spt-Server-Mod)<br>
...
3. Öffne die Server-Config im Servermod-Ordner `.../user/mods/SIT.Spt Server-Mod/config/coopConfig.json`.
...
6. Öffne die Serverconnection-Config im Server-Ordner in `.../SPT_Data/Server/configs/http.json`.
...
1. Tausche die Adresse bei `"ip": "127.0.0.1"` in den Dateien `<Server-Ordner>/SPT_Data/Server/configs/http.json`&
`<Server-Ordner>/SPT_Data/Server/database/server.json` mit der NIC-Adresse deines Servers aus oder nutze `0.0.0.0`
2. Tausche die Adresse bei `externalIP` in der `coopConfig.json` unter `<Server-Ordner>/user/mods/SIT.Spt-Server-Mod/config/` mit deiner Domain aus. z.B. `https://deinedomain.de:6969`
